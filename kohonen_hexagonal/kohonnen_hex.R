#################################################################################################
#                                                                                               #
#                                 Kohonen SOM with hexagonal bins                               #
#                                                                                               #
#################################################################################################
                            ...
                            keepMargins, ...)
{
  ...
  #numberofrows = length(table(x$grid$pts[,2]))
  #hexbg = matrix(bgcolors,ncol=numberofrows)
  #color2D.matplot(hexbg,cellcolors=bgcolors,main="test",do.hex=TRUE,...)
                         ...
                         heatkeywidth, ...)
{
  ...
  # DHP added this to try to have more control over the margins
  ...
  numberofrows = length(table(x$grid$pts[,2]))
  hexbg = matrix(property,ncol=numberofrows)
  # Define a function that maps numbers to colors
  
  # get nonzerovalues
  propertyclean =  property[which(is.finite(property))]
  
  # get colors
  breaks <- seq(min(propertyclean), max(propertyclean),by=(max(propertyclean) - min(propertyclean)) /ncolors   )
  cols <- palette.name(length(breaks) + 1)
  colvec = as.character(cut(property, c(-Inf, breaks, Inf), labels=cols))
  hexcol = matrix(colvec,ncol=numberofrows)
  
  #plot hexagons
  color2D.matplot(apply(t(hexbg), 2, rev),cellcolors=apply(t(hexcol), 2, rev),do.hex=TRUE,axes=FALSE,xlab="",ylab="",show.legend=FALSE,main=main,...)
