                            ...
                            scale = FALSE, heatkeywidth = .2, ...)
{
  ...
  switch(type,
         ...
         classes = plot.kohclasses(x = x, main = main,
           ...
           scale = scale, keepMargins = keepMargins, ...),
...
plot.kohclasses <- function(x, main, palette.name, bgcol,
                             codeRendering, property, scale, keepMargins, ...)
{
  ...
  if(!scale) {
    bgcol <- rep("gray", nrow(x$grid$pts))
  }
...
hexagon <- function(x,y,fg="black",bg="white") {
  ...
  polygon(X_vec, Y_vec, border="black", col=bg)
}
